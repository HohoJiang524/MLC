...
def predict_NB(X_train, X_test, y_train_i):
    nb = MultinomialNB()
...
def ClassifierChain_NB(data_train, data_test, label_train, label_test, savePath, ensemble=1):
    ...
    for i in range(ensemble):
        ...
        for index in order:
            ...
            pred, prob = predict_NB(X_train, X_test, y_train_i)
...
def BayesianClassifierChain_NB(data_train, data_test, label_train, label_test, savePath,
    ...
    if ensemble == 1:
        ...
        for label in order:
            ...
            pred, prob = predict_NB(X_tr, X_te, y_train_i)
    ...
    for j in range(5):
        ...
        for i in range(2):
            ...
            if methods.__name__ == "BayesianClassifierChain_NB":
                pred_ensemble, prob_ensemble = BayesianClassifierChain_NB(X_train, X_test, y_train, y_test, savePath,
                                                                             ensemble, ordering, structure, lead)
            elif methods.__name__ == "ClassifierChain_NB":
                pred_ensemble, prob_ensemble = ClassifierChain_NB(X_train, X_test, y_train, y_test,
                                                                     savePath, ensemble)
